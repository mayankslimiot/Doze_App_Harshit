...
export default function SuperAdminChangePassword() {
  ...
  useEffect(() => {
    const tempPass = sessionStorage.getItem('superadmin_setup_password');
    if (tempPass) {
  ...
  const onSubmit = async (data: SetupFormValues) => {
    setApiError('');
    try {
      const response = await fetch(apiUrl('/api/superadmin/change-password'), {
        method: 'POST',
      ...
      sessionStorage.removeItem('superadmin_setup_password');
  ...
  const handleContinueWithSamePassword = async () => {
    setApiError('');
    let currentPass = sessionStorage.getItem('superadmin_setup_password') || '';
    if (!currentPass) {
