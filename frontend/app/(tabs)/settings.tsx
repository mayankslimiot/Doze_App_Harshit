...
export default function SettingsScreen() {
  ...
  const userRole = auth.user?.role || auth.user?.profile?.role;
  const showOrgProvision = userRole === 'superadmin' || userRole === 'admin';

  const handleDeviceOnboarding = () => {
    const profile = auth.user?.profile;
    const account = profile?.account;
    const org = account?.organizationId || profile?.organizationId;
    ...
    if (userRole === 'admin' && org) {
      ...
      if (orgId) {
        setProvisionForHospitalId(orgId);
        setProvisionForHospitalName(orgName || 'My Organization');
        router.push('/(bluetooth)/ScanScreen');
        return;
      }
    }

    router.push('/hospital-provision');
  };
          ...
          {showOrgProvision && (
            <>
              ...
              <Row 
                ...
                subtitle="Assign a device to an organization"
                icon="business-outline"
                iconColor="rgba(0, 123, 144, 0.2)"
                onPress={handleDeviceOnboarding} 
                isLight={isLightTheme}
              />
            </>
          )}
