...
import { getHealthData, getHistoricalData, getDeviceHistory, getStressGraph, updateDeviceName, deleteDevice, activateDevice, removeSharedDevice, cleanupLocalBleStores } from '@/services/deviceData';
...
const TILE_WIDTH = (width - 40 - 12) / 2; // tile width
const SPARKLINE_WIDTH = TILE_WIDTH - 36; // leave slightly more padding (was 24)
...
export default function HomeScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('[Home] Screen focused — triggering refreshDevices');
      refreshDevices().catch((err) => console.warn('[Home] Auto-refresh devices failed:', err));
    }, [])
  );
  ...
  const fetchDeviceData = React.useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // Fetch latest health data and stress graph data (contains stress_level) in parallel
      const nowTime = new Date();
      const stressStart = new Date(nowTime.getTime() - (24 * 60 * 60 * 1000));
      const stressEnd = nowTime;

      const [healthResult, stressResult] = await Promise.all([
        getHealthData(activeDevice.deviceId, { limit: 1 }),
        getStressGraph(activeDevice.deviceId, stressStart, stressEnd)
      ]);

      let latestStress = 0;
      if (stressResult.success && stressResult.data?.points && stressResult.data.points.length > 0) {
        const latestStressPoint = [...stressResult.data.points]
          .reverse()
          .find((p: { x: number; y: number }) => p.y > 0 && p.y <= 100);
        if (latestStressPoint) {
          latestStress = latestStressPoint.y;
        }

        // Hydrate points to stress buffer so they are immediately available to the graph
        stressResult.data.points.forEach((point: { x: number; y: number }) => {
          if (point.x && point.y !== null && point.y !== undefined && point.y >= 0 && point.y <= 100) {
            addStressRawPoint(activeDevice.deviceId, point.x, point.y);
            if (point.x > nowTime.getTime() - (60 * 60 * 1000)) {
      ...
      // Calculate biological day (12 noon to 12 noon)
      const now = new Date();
      ...
      // Fetch historical data for charts
      const historyResult = await getDeviceHistory(activeDevice.deviceId, {
        from: dayStart,
        to: dayEnd,
        limit: 2000,
      });
      if (historyResult.success && historyResult.data) {
        // Sort chronologically to ensure sparklines render from left to right correctly
        const sortedData = [...historyResult.data].sort((a, b) => {
          const timeA = new Date(a.timestamp || a.timestampSeconds * 1000).getTime();
          const timeB = new Date(b.timestamp || b.timestampSeconds * 1000).getTime();
          return timeA - timeB;
        }).map(d => ({ ...d, _fromApi: true })); // Tag API data so we can distinguish from live WS data
        setHistoricalData(sortedData);
      } else {
        setHistoricalData([]);
      }

      // Fetch latest sleep session for the current active cycle
      ...
      // We can query sleep sessions from today 12 PM to targetDate 12 PM
      const cycleStart = new Date(targetDate);
      cycleStart.setDate(cycleStart.getDate() - 1);
      cycleStart.setHours(12, 0, 0, 0);

      const sleepResult = await getSleepSessions(activeDevice.deviceId, cycleStart, targetDate);
      if (sleepResult.success && sleepResult.data && sleepResult.data.length > 0) {
        // Find session that matches targetDateStr
        const session = sleepResult.data.find(s => s.sessionDate === targetDateStr) 
                     || sleepResult.data[sleepResult.data.length - 1];
        setActiveSleepSession(session);
      } else {
        // Fallback: calculate for active cycle
        const calcResult = await calculateSleepSession(activeDevice.deviceId, targetDateStr);
  ...
  React.useEffect(() => {
    ...
    const messageHandler = (data: any) => {
      ...
      setLatestHealthData((prev: any) => {
        ...
        // Handle stress data - prioritize new data (stress, stress_level, payload.stress_level), but keep previous if new is missing/invalid
        const incomingStress = data.stress ?? data.stress_level ?? (data.payload?.stress_level) ?? null;
        let parsedIncomingStress = incomingStress !== null ? (typeof incomingStress === 'string' ? parseFloat(incomingStress) : Number(incomingStress)) : null;
        if (parsedIncomingStress !== null && isNaN(parsedIncomingStress)) {
          parsedIncomingStress = null;
        }
        const newStress = parsedIncomingStress !== null
          ? parsedIncomingStress
          : (safePrev?.stress ?? 0);
      ...
      // Update historical data array (keep current biological day)
      setHistoricalData((prev) => {
        ...
        // Filter out old data points (before start of biological day)
  ...
  */
  ...
  const hasAnyDeviceData = React.useMemo(() => {
    ...
    // Also check that the data is from the current biological day (12 noon to 12 noon)
    // to avoid showing stale values from yesterday
    const ts = latestHealthData.timestamp;
    if (ts) {
      const dataTime = new Date(ts).getTime();
      const now = new Date();
      const dayStart = new Date(now);
      if (now.getHours() < 12) dayStart.setDate(dayStart.getDate() - 1);
      dayStart.setHours(12, 0, 0, 0);
      if (dataTime < dayStart.getTime()) return false; // data is from before today's biological day
    }
  ...
  const healthMetrics = React.useMemo(
    () => {
      // Only show live values if data is from the current biological day
      const hr = hasAnyDeviceData ? (latestHealthData?.heartRate || latestHealthData?.hr || 0) : 0;
      const resp = hasAnyDeviceData ? (latestHealthData?.respiration || latestHealthData?.resp || 0) : 0;
      const stress = hasAnyDeviceData ? (latestHealthData?.stress || 0) : 0;
    ...
    [latestHealthData, updateCounter, hasAnyDeviceData] // Include updateCounter to force recomputation
  );

  const envMetrics = React.useMemo(() => {
    // Only show live values if data is from the current biological day
    const temp = hasAnyDeviceData ? (latestHealthData?.temperature || latestHealthData?.temp || 0) : 0;
    const hum = hasAnyDeviceData ? (latestHealthData?.humidity || 0) : 0;
    const iaq = hasAnyDeviceData ? (latestHealthData?.iaq || latestHealthData?.raw?.AIR || 0) : 0;
    const tvoc = hasAnyDeviceData ? (latestHealthData?.tvoc || latestHealthData?.voc || 0) : 0;
    ...
    const getDiffStr = (key: string, current: number) => {
      if (!current || historicalData.length < 2) return null;
      const historyList = historicalData.map(d => {
        if (key === 'temp') return Number(d.temperature || d.temp || 0);
        if (key === 'hum') return Number(d.humidity || 0);
        if (key === 'iaq') return Number(d.iaq || (d.raw && (d.raw as any).AIR) || 0);
        return 0;
      }).filter(v => v > 0);
      
      let lastVal = current;
      for (let i = historyList.length - 1; i >= 0; i--) {
        if (historyList[i] !== current) {
          lastVal = historyList[i];
          break;
        }
      }
      if (lastVal === current) return null;
      
      const diff = current - lastVal;
      const isPositive = diff > 0;
      const sign = isPositive ? '+' : '-';
      const arrow = isPositive ? '↑' : '↓';
      const formattedDiff = Math.abs(diff).toFixed(2);
      const unitStr = key === 'temp' ? '°' : (key === 'hum' ? '%' : '');
      const diffColor = isPositive ? '#4CAF50' : '#F44336';
      
      return {
        text: `${arrow} ${sign}${formattedDiff}${unitStr} from last`,
        color: diffColor
      };
    };

    return [
      { key: 'temp', name: 'Temperature', value: temp > 0 ? temp.toFixed(2) : '--', unit: '°C', icon: '🌡️', colors: ['#2B2E57', '#1B1E3D'] as const, diff: getDiffStr('temp', temp), themeColor: isLightTheme ? '#0061A4' : '#4FC3F7' },
      { key: 'hum', name: 'Humidity', value: hum > 0 ? hum.toFixed(2) : '--', unit: '%', icon: '💧', colors: ['#24425F', '#18253A'] as const, diff: getDiffStr('hum', hum), themeColor: '#66BB6A' },
      { key: 'iaq', name: 'IAQ', value: iaq > 0 ? String(Math.round(iaq)) : '--', unit: '', icon: '🌬️', colors: ['#3A2C59', '#1D1430'] as const, diff: getDiffStr('iaq', iaq), themeColor: '#AB47BC' },
      { key: 'tvoc', name: 'TVOC', value: getTvocRating(tvoc), unit: '', icon: '☁️', colors: ['#2F2E4A', '#17162A'] as const, diff: null, themeColor: 'transparent' },
    ];
  }, [latestHealthData, updateCounter, historicalData, hasAnyDeviceData, isLightTheme]);
  ...
  const sparklines = React.useMemo(() => {
    ...
    const startMs = now.getTime() - rangeMs;

    // We will extract values with their timestamps from historicalData
    const dataMapWithTime: Record<string, Array<{t: number, v: number}>> = {
      heartRate: historicalData
        .map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.heartRate ?? d.hr ?? null) }))
        .filter(p => !isNaN(p.v) && p.v > 0 && p.t >= startMs),
      respiration: historicalData
        .map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.respiration ?? d.resp ?? null) }))
        .filter(p => !isNaN(p.v) && p.v > 0 && p.t >= startMs),
      stress: historicalData.map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.stress || 0) })).filter(p => p.v > 0 && p.t >= startMs),
      temp: historicalData.map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.temperature || d.temp || 0) })).filter(p => p.v > 0 && p.t >= startMs),
      hum: historicalData.map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.humidity || 0) })).filter(p => p.v > 0 && p.t >= startMs),
      iaq: historicalData.map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.iaq || (d.raw && (d.raw as any).AIR) || 0) })).filter(p => p.v > 0 && p.t >= startMs),
      tvoc: historicalData.map(d => ({ t: new Date(d.timestamp || (d as any).timestampSeconds * 1000).getTime(), v: Number(d.tvoc || (d as any).voc || 0) })).filter(p => p.v > 0 && p.t >= startMs),
    };

    [...healthMetrics, ...envMetrics].forEach((m) => {
      const dataPoints = dataMapWithTime[m.key] || [];
      if (dataPoints.length > 0) {
        ...
        const points = dataPoints.map((p, i) => {
          // Evenly space the points from left (0) to right (SPARKLINE_WIDTH) to always fill the full width of the card dynamically
          const x = dataPoints.length > 1 ? (i / (dataPoints.length - 1)) * SPARKLINE_WIDTH : SPARKLINE_WIDTH / 2;
          const y = max === min ? 14 : 28 - ((p.v - min) / range) * 28;
          return `${x},${y}`;
        }).filter(p => p && p.includes(',')); 

        if (points.length === 1) {
          // Draw a small dash in the center if there is only 1 point
          const center = SPARKLINE_WIDTH / 2;
          points[0] = `${center - 2},14`;
          points.push(`${center + 2},14`);
        }

        if (points.length >= 2) {
          map[m.key] = { svgPoints: points.join(' '), raw: values }; // values not shown in UI, raw array preserved
        }
      }
    });
  ...
  return (
    <View style={[styles.container, isLightTheme && { backgroundColor: '#F8F9FA' }]}>
      ...
      >
                    ...
                    >
                      <View style={styles.envTileInner}>
                        ...
                        <View style={styles.envValueRow}>
                          <Text key={`env-value-${m.key}-${updateCounter}`} style={[styles.envValueNum, isLightTheme && { color: '#111111' }]} adjustsFontSizeToFit numberOfLines={1}>{m.value}</Text>
                        ...
                        <View style={styles.sparklineWrap}>
                          {m.key === 'sleep' ? (
                            ...
                            hasAnyDeviceData ? <MiniSleepChart width={SPARKLINE_WIDTH} height={32} isLight={isLightTheme} /> : null
                          ) : m.key === 'stress' ? (
                            hasAnyDeviceData ? <MiniStressChart width={SPARKLINE_WIDTH} height={32} isLight={isLightTheme} /> : null
                          ) : (
                            <Svg height={28} width={SPARKLINE_WIDTH}>
                              {(() => {
        ...
        <BlurView
          intensity={0} // Remove blur as well to have no background effect
                  ...
                  return (
                      ...
                      >
                        <View style={styles.envTileInner}>
                          ...
                          {/* mini sparkline or diff text */}
                          <View style={styles.sparklineWrap}>
                            {(m.key === 'temp' || m.key === 'hum') ? (
                              m.diff ? (
                                <Text style={{ color: m.diff.color, fontSize: 13, fontWeight: '600' }}>{m.diff.text}</Text>
                              ) : (
                                <Text style={{ color: m.themeColor, fontSize: 13, fontWeight: '600', opacity: 0 }}>--</Text>
                              )
                            ) : (m.key === 'iaq' || m.key === 'tvoc') ? null : (
                              <Svg height={28} width={SPARKLINE_WIDTH}>
                                {(() => {
