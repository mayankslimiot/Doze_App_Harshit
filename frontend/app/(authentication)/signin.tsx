...
export default function SignInScreen() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (response.ok && result?.status === 'success' && result?.token && result?.user) {
        ...
        const userRole = String(result.user?.role ?? 'user');
        ...
        await login(token, { id: userId, email: userEmail, name: userName, role: userRole });
      ...
      if (error?.message?.toLowerCase().includes('network') || error?.message?.toLowerCase().includes('failed to fetch')) {
        errTitle = "The server is under maintenance.\nPlease login after an hour";
        errMsg = "";
      }
      triggerModal(errTitle, errMsg);
    } finally {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      let errTitle = "Google sign-in error";
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === 'ERR_REQUEST_CANCELED') {
        ...
        let errTitle = "Apple sign-in error";
        let errMsg = err?.message || 'An error occurred. Please try again.';
        if (errMsg.toLowerCase().includes('network') || errMsg.toLowerCase().includes('failed to fetch')) {
          errTitle = "The server is under maintenance.\nPlease login after an hour";
          errMsg = "";
