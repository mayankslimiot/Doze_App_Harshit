...
import { ActivityIndicator, Animated, Platform, SafeAreaView, ScrollView, StyleSheet, StatusBar, Text, TouchableOpacity, View, TextInput } from "react-native";
import { useTheme } from "@/contexts/ThemeContext";
import { Subscription } from 'react-native-ble-plx';
import { getWiFiStatus, autoRegisterDevice, updateDeviceName } from '@/services/deviceData';
import { 
...
export default function ConnectScreen() {
  const { selectedDeviceId, wifiSSID, wifiPassword, sendWifiCredentials, setWifiProvisioningSuccess, serialNumber, setSerialNumber, provisionForHospitalId } = useProvisioning();
    ...
    if (step2RegisterStatus === 'loading') {
      ...
      Animated.timing(step2Anim, {
        ...
        if (finished) {
          ...
          if (provisionForHospitalId) {
            setShowNameAssignment(true);
            // DO NOT show success state yet
          } else {
  ...
  const handleMQTTConnected = async () => {
    ...
    try {
      const registrationResult = await autoRegisterDevice(serialForRegistration, selectedDeviceId || undefined, provisionForHospitalId || undefined);
  ...
  const handleSaveDeviceName = async () => {
    if (!roomNumber.trim() || !bedNumber.trim()) return;
    setIsSavingName(true);
    const finalName = `doze_${roomNumber.trim()}_${bedNumber.trim()}`;
    const deviceId = serialNumberRef.current || serialNumber || selectedDeviceId;
    if (deviceId) {
      await updateDeviceName(deviceId, finalName, roomNumber.trim(), bedNumber.trim());
    }
        ...
        <View style={styles.headerIconContainer}>
          {showSuccessState && (
            <TouchableOpacity onPress={handleGoToDashboard}>
              <Ionicons name="close" size={28} color={isLightTheme ? "#111111" : "#FFF"} />
            </TouchableOpacity>
          )}
        </View>
      </View>
        ...
        <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
          ...
          {!showNameAssignment ? (
            ...
            <View style={{ width: '100%', alignItems: 'center', marginTop: 20 }}>
              <View style={[styles.stepCard, { width: '100%', alignItems: 'center', padding: 20 }]}>
                <Ionicons name="create-outline" size={48} color="#4A90E2" style={{ marginBottom: 10 }} />
                <Text style={[styles.stepCardTitle, isLightTheme && styles.stepCardTitleLight, { fontSize: 20, textAlign: 'center' }]}>
                  Assign Device Name
                </Text>
                <Text style={[styles.stepCardSubtitle, isLightTheme && styles.stepCardSubtitleLight, { textAlign: 'center', marginBottom: 20 }]}>
                  Device ID: {serialNumberRef.current || serialNumber || selectedDeviceId}
                </Text>
                ...
                <View style={{ width: '100%', marginBottom: 20 }}>
                  <TextInput
                    style={[{
                      ...
                      color: isLightTheme ? '#111111' : '#FFF',
                      borderRadius: 10,
                      padding: 15,
                      fontSize: 16,
                      borderWidth: 1,
                      borderColor: isLightTheme ? '#E5E7EB' : 'rgba(255, 255, 255, 0.1)',
                      width: '100%',
                      marginBottom: 10
                    }]}
                    placeholder="Room Number (e.g. 002)"
                  ...
                  <Text style={{ fontSize: 12, color: '#666', marginTop: 8 }}>
                    Will be saved as: doze_{roomNumber.trim() || '...'}_{bedNumber.trim() || '...'}
                  </Text>
                </View>

                <TouchableOpacity
                  style={[{
                    backgroundColor: (!roomNumber.trim() || !bedNumber.trim()) ? '#666' : '#4CAF50',
                    paddingVertical: 15,
                  ...
                  disabled={(!roomNumber.trim() || !bedNumber.trim()) || isSavingName}
                  onPress={handleSaveDeviceName}
                >
                  {isSavingName ? (
                    <ActivityIndicator size="small" color="#FFF" />
                  ) : (
                    <Text style={{ color: '#FFF', fontSize: 16, fontWeight: 'bold' }}>Save Name</Text>
                  )}
                </TouchableOpacity>
              </View>
            </View>
          ...
          <View style={{ minHeight: 160, width: '100%', alignItems: 'center', justifyContent: 'flex-start', marginTop: 10 }}>
            ...
            {showSuccessState && (
              <Animated.View style={{ 
                ...
                <TouchableOpacity
                  onPress={provisionForHospitalId ? handleAddMore : handleGoToDashboard}
                  style={[styles.dashboardButton, isLightTheme && styles.dashboardButtonLight]}
                >
                  <Ionicons name={provisionForHospitalId ? "add-circle-outline" : "home"} size={20} color="#FFF" />
                  <Text style={styles.dashboardButtonText}>{provisionForHospitalId ? "Add more" : "Go to Dashboard"}</Text>
                </TouchableOpacity>
              </Animated.View>
            )}
