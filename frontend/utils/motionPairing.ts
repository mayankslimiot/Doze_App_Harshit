 ...
 * 
 * Handles pairing of motionStart and motionStop values:
...
export interface MotionDataPoint {
  ...
  motionStop: number | null; // BIGINT, can be 0 (note: field name is motionStop, not motionEndTime)
}
...
export interface PairedMotionEvent {
  ...
  motionStop: number; // Unix timestamp when movement ended (in seconds)
...
export function pairMotionEvents(
  ...
  for (const point of dataPoints) {
    ...
    const motionStart = point.motionStart !== null && point.motionStart !== undefined && point.motionStart > 0 
      ...
      : null;
    const motionStop = point.motionStop !== null && point.motionStop !== undefined && point.motionStop > 0
      ? Number(point.motionStop)
      : null;
    ...
    if (sessionStartMs !== undefined && sessionEndMs !== undefined) {
      if (rowTimestamp < sessionStartMs || rowTimestamp > sessionEndMs) {
        ...
        if (motionStop !== null && openMovementStart !== null) {
          // Convert session times to seconds for comparison with motionStart/motionStop
          const sessionStartSeconds = sessionStartMs / 1000;
          if (motionStop > openMovementStart && openMovementStart >= sessionStartSeconds) {
            const durationSeconds = motionStop - openMovementStart; // Difference is in SECONDS
    ...
    if (motionStart !== null && motionStop !== null) {
        ...
        if (motionStart > openMovementStart) {
          ...
          if (durationSeconds > 0) {
            events.push({
              timestamp: openMovementStartTimestamp || (openMovementStart * 1000), // Convert to ms for display
              motionStart: openMovementStart,
              motionStop: motionStart, // Previous movement ends when new one starts
              durationSeconds,
              rowTimestamp,
            });
          }
      ...
      if (motionStop > motionStart) {
        const durationSeconds = motionStop - motionStart; // Difference is in SECONDS
        if (durationSeconds > 0) {
          events.push({
      ...
      // If motionStart > motionStop, it's invalid - skip
      continue;
    }
    ...
    // Handle motionStop (movement ends)
    if (motionStop !== null) {
      if (openMovementStart !== null) {
        // Validate: end must be after start
        if (motionStop > openMovementStart) {
          const durationSeconds = motionStop - openMovementStart; // Difference is in SECONDS
          if (durationSeconds > 0) {
            events.push({
              timestamp: openMovementStartTimestamp || (openMovementStart * 1000), // Convert to ms for display
