...
export function subscribe(
  ...
  const unsubscribeBuffer = require('./tvocBuffer').subscribeToBuffer(deviceId, (points: any[]) => {
    const now = Date.now();
    ...
    if (points.length > 0) {
      ...
      updateTvocGraphData(deviceId, currentZoom).catch((error: any) => {
        console.error('[TvocGraphManager] Error updating graph data:', error);
      });
    }
  });
