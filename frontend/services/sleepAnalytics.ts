...
export interface SleepSession {
  ...
  // ── Sleep Stage Data ─────────────────────────────────────────────────────
  // Per-epoch timeline from sleepStageService (30-sec epochs)
  stageTimeline?: Array<{
    ts: number;           // Unix timestamp seconds
    stage: 'AWAKE' | 'LIGHT' | 'DEEP' | 'REM';
    rawStage: 'AWAKE' | 'LIGHT' | 'DEEP' | 'REM';
    conf: number;         // confidence 0–100
    hr: number | null;    // heart rate for this epoch
    rr: number | null;    // respiration for this epoch
    tLastTurn: number;    // seconds since last body turn
  }>;
  // Stage summaries in minutes (0.5-min per 30-sec epoch)
  deepSleepMinutes?: number;
  remMinutes?: number;
  lightSleepMinutes?: number;
  awakeMinutes?: number;
  // Recovery % = (deep + REM epochs) / total sleep epochs × 100
  ...
  const requestPromise: Promise<SleepMotionTimelineResponse> = (async () => {
    try {
      ...
      const motionDataPoints = points
        .map((point: any) => {
          ...
          const motionStart = point?.motionStart !== null && point?.motionStart !== undefined
            ...
            : null;
          const motionStop = point?.motionStop !== null && point?.motionStop !== undefined
          ...
          return {
            ...
            motionStop: motionStop !== null && Number.isFinite(motionStop) ? motionStop : null,
          } as MotionDataPoint;
        })
      ...
      const events: SleepMotionEvent[] = pairingResult.events.map((event) => ({
        ...
        motionEndReason: event.motionStop,
