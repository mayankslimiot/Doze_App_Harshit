...
router.get("/data/health/weekly/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        let fieldNames = { primary: 'heartRate', secondary: 'hr' }; // Default
        if (metric === 'respiration') fieldNames = { primary: 'respiration', secondary: 'resp' };
        else if (metric === 'temperature') fieldNames = { primary: 'temperature', secondary: 'temp' };
        else if (metric === 'humidity') fieldNames = { primary: 'humidity', secondary: 'humidity' };
        else if (metric === 'iaq') fieldNames = { primary: 'iaq', secondary: 'raw.AIR' };
        else if (metric === 'tvoc') fieldNames = { primary: 'voc', secondary: 'voc' };
        else if (metric === 'eco2') fieldNames = { primary: 'co2', secondary: 'co2' };
                ...
                dayData = await HealthData180s.aggregate([
                    {
                        $match: {
                            deviceId: deviceId,
                            ...require('../utils/timeFilterHelper').getTimeFilter(dayStartUTC.getTime(), dayEndUTC.getTime()),
...
router.get("/data/health/raw/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        const { getTimeFilter } = require('../utils/timeFilterHelper');
        let timeFilter = {};
        if (start && end) {
            timeFilter = getTimeFilter(new Date(start).getTime(), new Date(end).getTime());
            Object.assign(query, timeFilter);
        } else if (start) {
            query.timestamp = { $gte: new Date(start) }; // Simple fallback for open-ended queries
        } else if (end) {
            query.timestamp = { $lte: new Date(end) }; // Simple fallback for open-ended queries
        }
...
router.get("/data/health/heart-rate/graph/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        const timeFilter = getTimeFilter(queryStart, queryEnd);
        ...
        .select("timestamp timestampSeconds heartRate hr bpm")
        .lean();

        const { getResolvedTimezone, formatGraphXLabel } = require('../utils/timezoneHelper');
        const timezone = getResolvedTimezone(req, user, device);
        ...
        if (raw === 'true') {
            ...
            const rawPoints = rawData.map(item => {
                const timestamp = getSanitizedEpochSeconds(item) * 1000;
                ...
                return {
                    x: timestamp,
                    xLabel: formatGraphXLabel(timestamp, timezone),
        ...
        const sanitizedData = rawData.map(item => ({
            ...item,
            timestamp: getSanitizedEpochSeconds(item) * 1000
        }));
        
        const graphData = aggregateHeartRateGraph(sanitizedData, zoomIndex, null, viewportWindow, timezone);
...
router.get("/data/health/respiration/graph/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        .sort({ timestampSeconds: 1, timestamp: 1 }) // Oldest first
        .select("timestamp timestampSeconds respiration resp")
        .lean();
        ...
        const graphData = aggregateRespirationGraph(sanitizedData, zoomIndex, null, viewportWindow, timezone);
...
router.get("/data/health/stress/graph/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        let queryStart, queryEnd;
        if (from && to) {
            queryStart = new Date(from).getTime();
            queryEnd = new Date(to).getTime();
        } else {
        ...
        const rawData = await HealthData180s.find(query)
            .sort({ timestampSeconds: 1, timestamp: 1 }) // Oldest first
            .select("timestamp timestampSeconds payload.stress_level")
            .lean();
        ...
        const points = rawData
            .map(doc => {
                ...
                const timestampMs = getSanitizedEpochSeconds(doc) * 1000;
                return {
                    timestamp: new Date(timestampMs).toISOString(), // ISO date string based on sanitized epoch
                    xLabel: formatGraphXLabel(timestampMs, timezone),
...
// ✅ Generic environmental graph endpoint builder
// Reusable factory for temperature, humidity, iaq (voc), eco2 (co2) graph endpoints
function buildEnvironmentalGraphRoute(routePath, dbField, maxValid, selectFields) {
    router.get(routePath, authMiddleware, async (req, res) => {
        try {
            const { deviceId } = req.params;
            const { zoomLevel = '0', raw = 'false', start, end } = req.query;

            const zoomIndex = parseInt(zoomLevel, 10);
            if (isNaN(zoomIndex) || zoomIndex < 0 || zoomIndex > 7) {
                return res.status(400).json({ success: false, message: "Invalid zoomLevel. Must be between 0 and 7" });
            }
            ...
            // Build query — include records where the field exists and has a valid value
            const matchQuery = {
                deviceId: deviceId,
                ...timeFilter
            };
            
            // Allow dbField to be an array for fallbacks
            if (Array.isArray(dbField)) {
                matchQuery.$or = dbField.map(field => ({
                    [field]: { $exists: true, $ne: null }
                }));
            } else {
                matchQuery[dbField] = { $exists: true, $ne: null };
            }

            // Also select timestampSeconds
            const actualSelectFields = selectFields.includes("timestampSeconds") ? selectFields : selectFields + " timestampSeconds";

            const rawData = await HealthData.find(matchQuery)
                .sort({ timestampSeconds: 1, timestamp: 1 })
            ...
            // If raw=true, return raw data points without aggregation
            if (raw === 'true') {
                const rawPoints = rawData
                    .filter(item => {
                        if (Array.isArray(dbField)) {
                            return dbField.some(field => field.includes('.') ? field.split('.').reduce((o, i) => o?.[i], item) != null : item[field] != null);
                        }
                        return item[dbField] != null;
                    })
                    .map(item => {
                        const timestampMs = getSanitizedEpochSeconds(item) * 1000;
                        let val = null;
                        if (Array.isArray(dbField)) {
                            for (const field of dbField) {
                                const fieldVal = field.includes('.') ? field.split('.').reduce((o, i) => o?.[i], item) : item[field];
                                if (fieldVal != null) {
                                    val = fieldVal;
                                    break;
                                }
                ...
                const timestamps = rawPoints.map(p => p.x).filter(t => Number.isFinite(t));
                const values = rawPoints.map(p => p.y).filter(v => v != null && Number.isFinite(v));

                const xDomain = timestamps.length > 0
                    ? [Math.min(...timestamps), Math.max(...timestamps)]
                    : [queryStart, queryEnd];

                // Dynamic Y domain based on actual data with padding
                const yMin = values.length > 0 ? Math.floor(Math.min(...values) - 2) : 0;
                const yMax = values.length > 0 ? Math.ceil(Math.max(...values) + 2) : 100;
            ...
            // Default: Aggregate using backend logic
            const { getTimeWindow } = require('../config/zoomLevels');
            const viewportWindow = getTimeWindow(zoomIndex);
            const { aggregateEnvironmentalGraph } = require('../utils/graphAggregation');
            ...
            const sanitizedData = rawData.map(item => {
                ...
                // aggregateEnvironmentalGraph expects the primary field to exist
                const primaryField = Array.isArray(dbField) ? dbField[0] : dbField;
                return {
                    ...item,
                    [primaryField]: val,
                    timestamp: getSanitizedEpochSeconds(item) * 1000
            ...
            const primaryField = Array.isArray(dbField) ? dbField[0] : dbField;
            const graphData = aggregateEnvironmentalGraph(sanitizedData, primaryField, zoomIndex, maxValid, null, viewportWindow, timezone);

            res.json({ success: true, data: graphData });
        } catch (error) {
            console.error(`Error fetching ${dbField} graph data:`, error);
            res.status(500).json({ success: false, message: "Server error", error: error.message });
        }
    });
}

// ✅ Temperature graph endpoint
buildEnvironmentalGraphRoute(
    "/data/health/temperature/graph/:deviceId",
    "temperature",
    80,  // max valid temperature = 80°C
    "timestamp temperature"
);

// ✅ Humidity graph endpoint
buildEnvironmentalGraphRoute(
    "/data/health/humidity/graph/:deviceId",
    "humidity",
    100, // max valid humidity = 100%
    "timestamp humidity"
);

// ✅ TVOC graph endpoint
buildEnvironmentalGraphRoute(
    "/data/health/tvoc/graph/:deviceId",
    "voc",
    10000, // max valid VOC reading
    "timestamp voc"
);

// ✅ IAQ graph endpoint
buildEnvironmentalGraphRoute(
    "/data/health/iaq/graph/:deviceId",
    ["iaq", "raw.AIR"],
    1000, // max valid IAQ reading
    "timestamp timestampSeconds iaq raw.AIR"
);

// ✅ ECO2 (CO2) graph endpoint
buildEnvironmentalGraphRoute(
    "/data/health/eco2/graph/:deviceId",
    "co2",
    10000, // max valid CO2 reading
    "timestamp co2"
);
...
router.get("/sleep/session/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        // ── Option A: Enrich response with snore data in one pass ───────────
        // Query HealthData for snore rows within the sleep window (non-blocking with Promise.all)
        let snoreDetail = null;
        try {
            const sleepStart = new Date(session.sleepOnsetTime);
            const sleepEnd   = new Date(session.sleepEndTime);

            if (sleepStart && sleepEnd && sleepEnd > sleepStart) {
                const snoreRows = await HealthData.find({
                    deviceId: resolvedDeviceId,
                    timestamp: { $gte: sleepStart, $lte: sleepEnd },
                    snoringStart: { $exists: true, $gt: 0 }
                })
                .select('snoringStart snoringStop snoringFrequency snoreDuration snoreCount maxSnoreScore timestampIST timestamp')
                .sort({ timestamp: 1 })
                .lean();

                if (snoreRows.length > 0) {
                    const totalEvents = snoreRows.length;

                    // A) Total raw seconds
                    const totalRawSeconds = snoreRows.reduce((sum, r) =>
                        sum + Math.max(0, (Number(r.snoringStop) || 0) - (Number(r.snoringStart) || 0)), 0);

                    // B) Avg event duration
                    const avgDuration = totalEvents > 0 ? totalRawSeconds / totalEvents : 0;

                    // D) Device snoreDuration (ms → sec)
                    const totalSnoreDurationSec = snoreRows.reduce((sum, r) =>
                        sum + (Number(r.snoreDuration) || 0) / 1000, 0);

                    // E) Per-frequency breakdown
                    const freqGroups = { 1: [], 2: [], 3: [] };
                    snoreRows.forEach(r => {
                        const f = [1, 2, 3].includes(Number(r.snoringFrequency))
                            ? Number(r.snoringFrequency) : 1;
                        freqGroups[f].push(r);
                    });

                    const freqBreakdown = [1, 2, 3].map(freq => {
                        const group = freqGroups[freq];
                        const eventCount  = group.length;
                        const share       = totalEvents > 0 ? Math.round((eventCount / totalEvents) * 1000) / 10 : 0;
                        const totalTimeSec = group.reduce((s, r) =>
                            s + Math.max(0, (Number(r.snoringStop) || 0) - (Number(r.snoringStart) || 0)), 0);
                        const avgMaxScore = eventCount > 0
                            ? Math.round(group.reduce((s, r) => s + (Number(r.maxSnoreScore) || 0), 0) / eventCount * 10) / 10
                            : 0;
                        const avgSnoreCount = eventCount > 0
                            ? Math.round(group.reduce((s, r) => s + (Number(r.snoreCount) || 0), 0) / eventCount * 100) / 100
                            : 0;
                        return { freq, eventCount, share, totalTimeSec, avgMaxScore, avgSnoreCount };
                    });

                    // F) Snore Timeline (exact timestamps for plotting along sleep timeline)
                    const snoreTimeline = snoreRows.map(r => ({
                        start: Number(r.snoringStart) || 0,
                        duration: Math.max(0, (Number(r.snoringStop) || 0) - (Number(r.snoringStart) || 0)),
                        freq: Number(r.snoringFrequency) || 1
                    })).filter(e => e.start > 0);

                    // G) Frequency distribution %
                    const fc1 = freqGroups[1].length, fc2 = freqGroups[2].length, fc3 = freqGroups[3].length;
                    const freq1Pct = totalEvents > 0 ? (fc1 / totalEvents) * 100 : 0;
                    const freq2Pct = totalEvents > 0 ? (fc2 / totalEvents) * 100 : 0;
                    const freq3Pct = totalEvents > 0 ? (fc3 / totalEvents) * 100 : 0;

                    snoreDetail = {
                        totalEvents,
                        totalRawSeconds,
                        avgDuration: Math.round(avgDuration * 100) / 100,
                        totalSnoreDurationSec: Math.round(totalSnoreDurationSec * 100) / 100,
                        freqBreakdown,
                        snoreTimeline,
                        freqCounts : { fc1, fc2, fc3 },
                        freqPct    : { freq1Pct, freq2Pct, freq3Pct }
                    };
                }
            }
        } catch (snoreErr) {
            // Non-fatal — snore enrichment failure must not break the sleep session response
            console.error('[SnoreDetail] Failed to compute snore detail (non-fatal):', snoreErr.message);
        }

        const sessionData = session.toObject ? session.toObject() : session;
        res.json({
            success: true,
            data: { ...sessionData, snoreDetail }
        });
...
router.get("/data/sleep/:deviceId", authMiddleware, async (req, res) => {
    try {
        ...
        const { getResolvedTimezone, formatSessionTime } = require('../utils/timezoneHelper');
        const timezone = getResolvedTimezone(req, null, null); // Rely on X-Timezone header

        const formattedSessions = sleepSessions.map(session => ({
            ...session,
            formattedSessionStart: formatSessionTime(session.sleepOnsetTime, timezone),
            formattedSessionEnd: formatSessionTime(session.sleepEndTime, timezone)
        }));
